...
#define OUTPUT_FILE "./IO/output.aiff"
...
char *audio_converter(char *type) {
    ...
    if (strcmp("-mp3", type) == 0) {
        strcpy(audio, "./IO/output.mp3");
    } else if (strcmp("-wav", type) == 0) {
        strcpy(audio, "./IO/output.wav");
    }
    audio[10] = '\0';
    return audio;
}
    ...
    if (pid < 0) {
        ...
        if (argc == 3) {
            ...
            if (pid_con == 0) {
                ...
                execl(FFMPEG_PATH, "ffmpeg","y", "-i", OUTPUT_FILE, out_file, NULL);
        ...
        execl(SAY_PATH, "say", OUTPUT_VOICE, "-o", OUTPUT_FILE, buffer, NULL);
